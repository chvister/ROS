...
if __name__ == '__main__':
    ...
    image_sub = rospy.Subscriber("/camera/rgb/image_raw", Image, img_cb)

    printed_number = 2
    max_number     = 2  # if this number is reached program is closed file is written.
    ...
    while not rospy.is_shutdown():
        ...
        if (printed_number<=max_number):
            ...
            try:
                ...
                (trans, rot) = listener.lookupTransform('/camera/rgb/image_raw', tf_to_str, rospy.Time(0))

                # get id of tf_to_str
                for p in list_frames:
                    if "id_" in p:
                        (trans_p, rot_p) = listener.lookupTransform('/camera/rgb/image_raw', p, rospy.Time(0))
                ...
                if(len(id_nr)>1):
                    ...
                    str_output = "<node name=\"initpos_to_globe_"+str(printed_number)+"_map_tf_pub\" pkg=\"tf\" type=\"static_transform_publisher\" args=\""+str_output+" /odom  /"+id_nr+"_map 100 \"/>"
